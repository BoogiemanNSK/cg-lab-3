...
void Renderer::LoadPipeline()
{
	...
	CreateDXGIFactory2(dxgi_factory_flag, IID_PPV_ARGS(&dxgi_factory));
	...
	rtv_heap_descriptor.NumDescriptors = 1;
	rtv_heap_descriptor.Type = D3D12_DESCRIPTOR_HEAP_TYPE_CBV_SRV_UAV;
	rtv_heap_descriptor.Flags = D3D12_DESCRIPTOR_HEAP_FLAG_SHADER_VISIBLE;
	ThrowIfFailed(device->CreateDescriptorHeap(&rtv_heap_descriptor, IID_PPV_ARGS(&cbv_heap)));
}
...
void Renderer::LoadAssets()
{
	...
	pso_desc.InputLayout.pInputElementDescs = input_element_descriptors;
	pso_desc.InputLayout.NumElements = _countof(input_element_descriptors);
	...
	device->CreateCommandList(0, D3D12_COMMAND_LIST_TYPE_DIRECT, command_allocators[0].Get(), pipeline_state.Get(), IID_PPV_ARGS(&command_list));
